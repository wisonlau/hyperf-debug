 ...
 * @version 1.0
 */
class Hpdebug implements OnMessageInterface, OnOpenInterface, OnCloseInterface
{
    ...
    {
        $config = config('hpdebug') ?? '';
        $this->host = isset($config['debughost']) && $config['debughost'] ? $config['debughost'] : 'ws://127.0.0.1:9502';
    ...
    {
        $file =  @file_get_contents(__DIR__ . '/../view/Debug.html');
        $file = str_replace('{{debughost}}',$this->host, $file);
        return $file;
    }
